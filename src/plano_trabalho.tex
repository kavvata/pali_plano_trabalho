...
Para que a solução projetada se ajuste a esse trabalho real, utilizam-se duas técnicas de representação do usuário: as personas e o mapa de empatia \cite{siricharoen_using_2021}. O mapa de empatia organiza os dados da pesquisa em torno do que o usuário pensa, sente, vê, ouve e faz, permitindo compreender sua experiência e suas intenções para além do registro de comportamentos. A persona, por sua vez, consolida essas evidências em um perfil arquetípico construído a partir de dados empíricos, tratando o usuário como uma personagem coerente -- e não como um conjunto de especificações --, o que facilita a comunicação das decisões de design. Os dois conceitos são complementares e sequenciais na estrutura do DT: o mapa de empatia, produzido na fase de empatia, fornece as evidências que a fase de definição consolida na persona \cite{siricharoen_using_2021}. 

Dessas evidências, a definição converge ainda para o Point of View (POV), formulação estruturada do problema que articula a descrição do usuário, a necessidade identificada e o insight -- a razão subjacente, muitas vezes não explicitada, que torna essa necessidade relevante \cite{rodriguez_point_2019}. Ao articular esses três elementos, o POV evita tanto a generalidade de um problema mal definido quanto a especificidade prematura de uma solução, delimitando o espaço de ideação da etapa seguinte.

TODO: Análise Temática e Ontologia

TODO: Engenharia de software, requisitos funcionais e nao funcionais

Ao converter o problema delimitado em decisões de engenharia, define-se a especificação de requisitos do sistema \cite{sommerville_software_2016}. Os requisitos funcionais especificam os serviços que o sistema deve oferecer e seu comportamento frente a eventos externos -- aqui, o registro de comunidades ecológicas, a triagem e a identificação de táxons e a gestão dos dados a eles associados. Os requisitos não funcionais, por sua vez, estabelecem as restrições e os atributos de qualidade que a solução deve satisfazer -- desempenho, usabilidade, confiabilidade e manutenibilidade --, sendo tão determinantes quanto os funcionais para que o sistema se ajuste ao fluxo de trabalho do pesquisador. O produto dessa atividade é o documento de especificação de requisitos, referência para o projeto, o desenvolvimento e a aceitação do sistema.
