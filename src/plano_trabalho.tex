...
Apesar da relevância desse monitoramento, os métodos atualmente empregados na triagem e catalogação de táxons em comunidades ecológicas permanecem, em grande parte, manuais e sem gestão de informações. Por exemplo, em um estudo dirigido por \citeonline{luciane_marinoni_colecoes_2025} evidenciou que, no Brasil, cerca de 71,5\% das coleções zoológicas não utilizam de nenhum sistema para a gestão das coleções. A ausência dessa gestão de dados -- fatos conhecidos que podem ser guardados e possuem um significado implícito \cite{elmasri_fundamentals_2017} --  via sistema pode levar ao comprometimento da padronização de informações coletadas, o que aumenta o tempo da análise e prejudica a qualidade dos resultados \cite{specht_data_2015}.
